...
qhyccd = ctypes.CDLL(r'd:\TestUroki\studies\studies\test_project\pyton\qhyccd.dll')
...
#------------серийное фото-----------
while True:
    response = qhyccd.GetQHYCCDSingleFrame( #сделать фотографию - в скобках параметры снимка
    camera_handle, ctypes.byref(maxImageSizeX), ctypes.byref(maxImageSizeY), 
    ctypes.byref(depth), ctypes.byref(channels), image_data,
)
    ...
    mono_image = raw_array.reshape(maxImageSizeY.value, maxImageSizeX.value) #Изменить размер снимка под размер сенсора камеры
    cv2.imshow("camera", mono_image )
...
cv2.imwrite('frame.bmp', mono_image) # Записать полученое фото на HDD
